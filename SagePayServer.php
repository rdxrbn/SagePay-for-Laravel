...
class SagePayServer {
    ...
    public function __construct($vendor='vendor', $mode='test') {
    ...
    public function register() {
        ...
        if ($this->result['Status'] == '3DAUTH') {
            //session(['sagepay_obj' =>  serialize($this)]);
            Session::put('sagepay_obj', serialize($this));
        }
    ...
    public static function recover3d() {
        $sagepay = unserialize(Session::get('sagepay_obj'));
        Session::forget('sagepay_obj');
        return $sagepay;
    }

    public static function is3dResponse($request) {
        $PaRes=$request['PaRes'];
        $MD=$request['MD'];
        $sagepay_obj=Session::get('sagepay_obj');
        if (isset($PaRes) and isset($MD) and isset($sagepay_obj)) {
            return true;
        } else {
            return false;
        }
    ...
    public function complete3d($request) {
        $data = array(
            'PARes' => $request['PaRes'],
            'MD'    => $request['MD']
        );
